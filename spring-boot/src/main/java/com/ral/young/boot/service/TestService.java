...
public class TestService {
    ...
    private boolean taskOneEnable = false;

    private boolean taskTwoEnable = false;

    private boolean taskThreeEnable = false;

    private boolean taskFourEnable = false;
    ...
    private ResponseEntity<JSONObject> getResponse(Result result) {
        ...
        headers.add("x-auth-token", "w0WpBNNCxifsePqFVJ6Kv_n9d9cIWN7DAFd30V7E");
    ...
    private static Result getResult(String data) {
        ...
        taskId = replaceLastSix(taskId, true, 6);
        Long timeStamp = DateUtil.current();
        String eventId = jsonObject.getStr(EVENT_ID);
        eventId = replaceLastSix(eventId, false, 6);
