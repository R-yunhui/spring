...
![image-20220725142201449](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725142201449.png)
...
短连接：每次执行完很少的几次查询就断开连接，下次查询再重新建立一个。（建立连接的过程通常是比较复杂的，建议你在使用中要尽量减少建立连接的动作，也就是尽量使用长连接。）
...
![image-20220725143109942](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725143109942.png)
...
![image-20220110155630462](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220110155630462.png)
...
![image-20220110155829873](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220110155829873.png)
...
![InnoDB刷脏页速度策略](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/InnoDB刷脏页速度策略.png)
...
```
如上图：每次事务提交都要写 redo log，如果设置太小，很快就会被写满，也就是上面这个图的状态，这个“环”将很快被写满，write pos 一直追着 CP。这时候系统不得不停止所有更新，去推进 checkpoint。（因为 checkpoint 一直要往前推，这个操作就会触发 merge 操作，然后又进一步地触发 flush 操作,导致 change buffer 的优化也失效了）
...
![image-20220725145123833](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725145123833.png)
...
![一个记录被多个事务连续更新后的状态](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/一个记录被多个事务连续更新后的状态.png)
...
![image-20220109143524342](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109143524342.png)
...
![image-20220109143903937](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109143903937.png)
...
![image-20220109144416141](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109144416141.png)
...
![image-20220109145337997](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109145337997.png)
...
![image-20220109150208028](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109150208028.png)
...
![image-20220109152733176](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109152733176.png)
...
![Innodb 的所有索引组织结构](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/Innodb 的所有索引组织结构.png)
...
![（name，age）索引示意图](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/（name，age）索引示意图.jpg)
...
![无索引下推执行流程](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/无索引下推执行流程.jpg)

​																														**图1.无索引下推过程**

![索引下推执行流程](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/索引下推执行流程.jpg)
...
![image-20220109160143285](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109160143285.png)
...
![image-20220109160625735](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220109160625735.png)
...
![image-20220110151733607](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220110151733607.png)

​																										**图 1 email 索引结构**



![image-20220110151754660](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220110151754660.png)
...
12.简单地对比这两个机制（change buffer 和 redo log）在提升更新性能上的收益的话，redo log 主要节省的是随机写磁盘的 IO 消耗（转成顺序写），而 change buffer 主要节省的则是随机读磁盘的 IO 消耗。
...
```
全局锁是对整个数据库实例加锁：flush tables with read locak；让整个库处于只读状态，之后其他线程的以下语句会被阻塞：数据更新语句（数据的增删改）、数据定义语句（包括建表、修改表结构等）和更新类事务的提交语句。
...
MySQL 5.5 版本中引入了 MDL，当对一个表做增删改查操作的时候 - DML语句，加 MDL 读锁；当要对表做结构变更操作的时候 - DDL 语句，加 MDL 写锁。
...
![MDL锁](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/MDL锁.jpg)
...
![Mysql的死锁](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/Mysql的死锁.jpg)
...
![image-20220111162655612](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220111162655612.png)
...
![插入数据导致页分裂](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/插入数据导致页分裂.png)
...
![image-20220111163918260](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220111163918260.png)
...
![image-20220111164124301](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220111164124301.png)
...
```
1.delete 语句删除表的数据只是打了一个可复用的标记，如果是数据页上的一部分数据打了标记，对应自增主键的 insert 数据，这部分被标记可复用的数据页空间是不会被服用的，如果是整个数据页都被打上了可复用的标记，那么是可以复用的。
...
![image-20220112133840606](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220112133840606.png)
...
![image-20220112152212709](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220112152212709.png)
...
![image-20220112152333090](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220112152333090.png)
...
![image-20220112152701266](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220112152701266.png)
...
![image-20220113153938178](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113153938178.png)
...
![image-20220113154125236](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113154125236.png)
...
![image-20220113154527126](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113154527126.png)
...
![image-20220113155026930](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113155026930.png)
...
![image-20220113155418298](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113155418298.png)
...
![image-20220113155506803](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113155506803.png)
...
![image-20220113155903203](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113155903203.png)

​																					**图 7 city 和 name 联合索引示意图**

![image-20220113155956859](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113155956859.png)
...
![image-20220113160123143](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113160123143.png)
...
![image-20220113160354239](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113160354239.png)

​																		**图 10 引入 (city,name,age) 联合索引后，查询语句的执行流程**

![image-20220113160459941](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220113160459941.png)
...
![image-20220115122535096](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115122535096.png)
...
![image-20220115123853254](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115123853254.png)
...
![image-20220115124628862](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115124628862.png)
...
![image-20220115124741572](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115124741572.png)
...
![image-20220115130440771](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115130440771.png)
...
![image-20220115130856015](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115130856015.png)
...
![image-20220115165316195](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115165316195.png)
...
![image-20220115165710385](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115165710385.png)
...
![image-20220115170058437](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115170058437.png)
...
![image-20220115170145232](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115170145232.png)

​																				**图 4 Waiting for table flush 状态示意图**

![image-20220115170233552](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115170233552.png)
...
![image-20220115170329182](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115170329182.png)

​																										**图 6 行锁复现**

![image-20220115170540622](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115170540622.png)
...
![image-20220115171639039](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115171639039.png)

​																													**图 8 复现步骤**

![image-20220115171706151](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220115171706151.png)
...
![image-20220116115608886](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220116115608886.png)
...
![image-20220117134843972](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117134843972.png)
...
![image-20220117134952506](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117134952506.png)
...
![image-20220117135207927](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117135207927.png)
...
![image-20220117133653574](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117133653574.png)
...
![image-20220117134131324](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117134131324.png)
...
![image-20220117134647839](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220117134647839.png)
...
![image-20220725152020331](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725152020331.png)
...
![image-20220725152128212](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725152128212.png)
...
![redo log 组提交](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/redo%20log%20%E7%BB%84%E6%8F%90%E4%BA%A4.png)
...
![image-20220725152445184](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/image-20220725152445184.png)
...
![两阶段提交细化](https://renyh-bucket-1302033757.cos.ap-chengdu.myqcloud.com/images/%E4%B8%A4%E9%98%B6%E6%AE%B5%E6%8F%90%E4%BA%A4%E7%BB%86%E5%8C%96.png)
...
![image-20220112165854068](http://ren-bed.oss-cn-beijing.aliyuncs.com/img/image-20220112165854068.png)
